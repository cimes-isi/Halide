    ...
    Stmt visit(const For *op) override {
        if (op->for_type == ForType::Vectorized) {
            return For::make(op->name, op->min, op->extent,
                             ForType::Serial, op->distributed, op->device_api, mutate(op->body));
        }
...
class VectorSubs : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        if (min.type().is_vector()) {
            ...
            Stmt transformed = For::make(op->name, 0, op->extent, for_type, op->distributed, op->device_api, body);
        ...
        if (op->for_type == ForType::Vectorized) {
            ...
            if (min.same_as(op->min) &&
                ...
                return For::make(op->name, min, extent, for_type, op->distributed, op->device_api, body);
            }
        }
    }
    ...
    Stmt scalarize(Stmt s) {
        ...
        for (int ix = vectorized_vars.size() - 1; ix >= 0; ix--) {
            s = For::make(vectorized_vars[ix].name, vectorized_vars[ix].min,
                          vectorized_vars[ix].lanes, ForType::Serial, false /* distributed */, DeviceAPI::None, s);
        }
