...
class SlidingWindowOnFunctionAndLoop : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        if (is_const_one(extent)) {
            ...
            internal_assert(l);
            return For::make(op->name, op->min, op->extent, op->for_type, op->distributed, op->device_api, l->body);
...
class SlidingWindowOnFunction : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        if (new_body.same_as(op->body)) {
            ...
            return For::make(op->name, op->min, op->extent, op->for_type, op->distributed, op->device_api, new_body);
        }
    }
